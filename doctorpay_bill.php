...
session_start();
	...
	if (!isset($user_id)) {
	header('location: doctor/Doctor_login.php?paction=not_login');
	}
		...
		if (empty(trim($Exp_Year))) {
			...
			if (empty($a) and empty($b) and empty($c) and empty($d) and empty($e)) {
						...
						if ($result_set) {
								...
								header('location: doctor/my_appointment.php?Payment=successfully');
							}
...
<div class="jumbotron bg-light border border-dark" style="margin:500px;margin-top:20px; width:600px;">
  ...
  <form action="doctorpay_bill.php?var1=<?= $BNU ?>&Book_Now=<?= $_GET['Book_Now'] ?>" method="post">
  		<div class="row">
              <div class="col-50">
              <h4>Your Details</h4>
